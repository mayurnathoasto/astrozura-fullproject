...
export default function RitualDetail() {
  ...
  const handleBook = () => {
    if (primaryAstrologer?.id) {
      navigate(`/consultation/${primaryAstrologer.id}`, { state: { type: "call", astrologer: primaryAstrologer } });
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#faf8f3] text-[#1E3557]">
      ...
      <section className="relative overflow-hidden bg-[#13294b] text-white">
        ...
        <img src={getImageUrl(ritual.image, ritualFallbacks[0])} alt={ritual.name} className="absolute inset-0 h-full w-full object-cover opacity-25" />
        <div className="relative max-w-7xl mx-auto px-4 md:px-8 py-16">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#D4A73C]">{ritual.category}</p>
          <h1 className="mt-4 text-4xl md:text-5xl font-black max-w-3xl">{ritual.name}</h1>
          <p className="mt-4 max-w-2xl text-sm md:text-base leading-7 text-slate-200">{ritual.short_description}</p>
          <div className="mt-6 flex flex-wrap gap-4 text-sm">
            ...
            <span className="rounded-full bg-[#D4A73C] px-4 py-2 font-bold text-[#1E3557]">Rs {Number(ritual.price || 0).toLocaleString("en-IN")}</span>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 py-10">
        <div className="grid gap-8 xl:grid-cols-[1fr_320px]">
              ...
              <div className="rounded-3xl bg-white p-6 shadow-sm border border-[#efe4d2]">
                ...
                <div className="mt-4 space-y-3">
                  {(ritual.mantras || []).map((item, index) => (
                    <div key={`${item}-${index}`} className="rounded-2xl bg-[#fffaf0] px-4 py-3 text-sm leading-6 text-gray-600">
                      {item}
                    </div>
                  ))}
                </div>
            ...
            {!!similarRituals.length && (
              <div className="rounded-3xl bg-white p-6 shadow-sm border border-[#efe4d2]">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <Link to="/rituals" className="text-sm font-semibold text-[#D4A73C]">View all rituals</Link>
                </div>
                <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  {similarRituals.map((item, index) => (
                    <Link key={item.id} to={`/rituals/${item.slug}`} className="rounded-2xl border border-gray-100 overflow-hidden">
                      <img src={getImageUrl(item.image, ritualFallbacks[index % ritualFallbacks.length])} alt={item.name} className="h-28 w-full object-cover" />
                      <div className="p-3">
                        <p className="text-sm font-semibold leading-5">{item.name}</p>
                        <p className="mt-1 text-xs text-[#D4A73C]">Rs {Number(item.price || 0).toLocaleString("en-IN")}</p>
                      </div>
                    </Link>
                  ))}
                </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-3xl bg-white p-5 shadow-sm border border-[#efe4d2] sticky top-24">
              {primaryAstrologer && (
                <div className="rounded-2xl border border-gray-100 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#D4A73C]">Ritual Guide</p>
                  <div className="mt-4 flex items-center gap-3">
                    <img src={getImageUrl(primaryAstrologer.astrologer_detail?.profile_image, astrologerFallbacks[0])} alt={primaryAstrologer.name} className="h-14 w-14 rounded-full object-cover" />
                    <div>
                      <p className="font-semibold">{primaryAstrologer.name}</p>
                      <p className="text-xs text-gray-500">{primaryAstrologer.astrologer_detail?.specialities || "Vedic Ritual Specialist"}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="mt-5 rounded-2xl bg-[#fffaf0] p-4">
                ...
                <div className="mt-4 space-y-3 text-sm text-gray-600">
                  <div className="flex justify-between"><span>Duration</span><span className="font-semibold text-[#1E3557]">{ritual.duration_label}</span></div>
                  <div className="flex justify-between"><span>Mode</span><span className="font-semibold text-[#1E3557]">{ritual.mode}</span></div>
                  <div className="flex justify-between"><span>Ideal Timing</span><span className="font-semibold text-right text-[#1E3557]">{ritual.ideal_timing}</span></div>
                  <div className="flex justify-between"><span>Total Amount</span><span className="font-bold text-[#1E3557]">Rs {Number(ritual.price || 0).toLocaleString("en-IN")}</span></div>
                </div>
                <button type="button" onClick={handleBook} className="mt-5 w-full rounded-xl bg-[#D4A73C] px-4 py-3 text-sm font-bold text-[#1E3557]">
                  Book Now
                </button>
              </div>
                  ...
                  <div className="mt-3 space-y-3">
                    {recommendedAstrologers.map((astro, index) => (
                      <button
                        ...
                        className="flex w-full items-center gap-3 rounded-2xl border border-gray-100 px-3 py-3 text-left hover:border-[#D4A73C] transition"
                      >
                        <img src={getImageUrl(astro.astrologer_detail?.profile_image, astrologerFallbacks[index % astrologerFallbacks.length])} alt={astro.name} className="h-11 w-11 rounded-full object-cover" />
                        <div className="min-w-0">
                          <p className="truncate font-semibold">{astro.name}</p>
                          <p className="truncate text-xs text-gray-500">{astro.astrologer_detail?.specialities || "Astrology Specialist"}</p>
                        </div>
                      </button>
                    ))}
                  </div>
